...
def ask_invoice_question(question: str) -> str:
    ...
    try:
        ...
        print(f"Error in ask_invoice_question: {error_details}")  # For debugging
        return f"Failed to answer question: {str(e)}. Please check if your GROQ_API_KEY is properly set in the environment variables."
...
def generate_search_queries_with_llm(question: str) -> list:
    ...
    try:
        ...
        # Read from env only; do not fall back to any hardcoded secret
        ...
        import traceback
        error_details = traceback.format_exc()
        print(f"Error in generate_search_queries_with_llm: {error_details}")  # For debugging
...
def generate_llm_response(question: str, nodes) -> str:
    ...
    try:
        ...
        import traceback
        error_details = traceback.format_exc()
        print(f"Error in generate_llm_response: {error_details}")  # For debugging
...
def generate_template_response(question: str, content: str, nodes) -> str:
    ...
    # Provide a more helpful response with available information
    if not nodes:
        return "I couldn't find any relevant information in the invoices to answer your question. Please try rephrasing your question or check if the invoices have been properly ingested."
    
    # Show what information we found
    filenames = [node.metadata.get('file_name', 'unknown') for node in nodes]
    unique_files = list(set(filenames))
    
    response = f"I found information in {len(unique_files)} invoice(s): {', '.join(unique_files)}. "
    response += "However, I'm having trouble processing your specific question right now. "
    response += "Please try rephrasing your question or be more specific about what you're looking for."
